...
/*  release user_proc's resoureces, including:
    1. physical pages
    2. vitual memory pool
    3. files opened */
static void release_prog_resource(struct task_struct* release_thread){
    uint32_t* pgdir_vaddr = release_thread->pgdir;
    uint16_t user_pde_nr = 768, pde_idx = 0;
    uint32_t pde = 0;
    uint32_t* v_pde_ptr = NULL; //v stands for var
    ...
    //pointes to the first pte's pointing physical address
    uint32_t pg_phy_addr = 0;

    /*release page_dir's containing memory*/
    while(pde_idx < user_pde_nr){
        v_pde_ptr = pgdir_vaddr + pde_idx;
        pde = *v_pde_ptr;
        if(pde & 0x00000001){
            //unempty page tables may present 
            first_pte_vaddr_in_pde = pte_ptr(pde_idx * 0x400000);
            //one page table holds 4mb, which is 0x400000
            pte_idx = 0;
    ...
    /*release physical memory occupied by process's virtual memory pool*/
...
/*  used by list_traversal to find child process by comparing pid of this process and ppid
    of checking process*/
...
/*  checking child process with status TASK_HANGING as the function above*/
...
/*  used by list_traversal, this function adds a child process to init's children*/
...
/*  wait untile child called exit. set child process's exiting status to status 
    return child's pid if success, -1 if failed*/
pid_t sys_wait(int32_t* status){
    struct task_struct* parent_thread = running_thread();

    while(1){
        /*handling hanging tasks first*/
        ...
        if(child_elem != NULL){
            ...
            *status = child_thread->exit_status;
            ...
            /*acquire pid before pcb is released*/
            uint16_t child_pid = child_thread->pid;

            /*exit child thread*/
            thread_exit(child_thread, false);
            // 传入false,使thread_exit调用后回到此处
	        /* 进程表项是进程或线程的最后保留的资源, 至此该进程彻底消失了 */
            return child_pid;
        }
        /*checking if there is a child process*/
        child_elem = list_traversal(&thread_all_list, find_child, parent_thread->pid);
        if(child_elem == NULL){
            return -1;
        }else{
            thread_block(TASK_WAITING);
            //hanging itself if child process is still running
        }
    }
}
...
void sys_exit(int32_t status){
    ...
    /*hand over all child of this process to init()*/
