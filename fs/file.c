...
/* 以下开始构建all_blocks块地址数组,专门存储用到的块地址(本程序中块大小同扇区大小) */
   ...
   /* 第一种情况: 起始块和终止块属于直接块*/
      if (block_read_end_idx < 12 ) {	  // 数据结束所在的块属于直接块
        ...
        while (block_idx <= block_read_end_idx) {
          all_blocks[block_idx] = file->fd_inode->i_sectors[block_idx];
       //   printf("this is important: %d, %d\n", count, file->fd_inode->i_sectors[block_idx]); 
          block_idx++;
	      }
  ...
  // for(int i = 0; i < 10; i++){
  //    printf("this is important %d, %d\n",file->fd_inode->i_sectors[i], count);
  //  }
   ...
   while (bytes_read < size) {	      // 直到读完为止
      ...
      sec_lba = all_blocks[sec_idx];
      //printf("this is important %d, %d\n",all_blocks[sec_idx], count);
      sec_off_bytes = file->fd_pos % BLOCK_SIZE;
      sec_left_bytes = BLOCK_SIZE - sec_off_bytes;
      chunk_size = size_left < sec_left_bytes ? size_left : sec_left_bytes;	     // 待读入的数据大小

      memset(io_buf, 0, BLOCK_SIZE);
      if(sec_idx < 12 && sec_lba > ((80*1024*1024/512) - 1)){ //max_lba. prevent over define
        sec_lba = file->fd_inode->i_sectors[sec_idx];
      }
      //printf("%d, I am here, %d\n", sec_lba, count);
      ide_read(cur_part->my_disk, sec_lba, io_buf, 1);
