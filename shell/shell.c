...
void my_shell(void) {
   ...
   while(1){
	   ...
	   if(pipe_symbol){
		   ...
		   int32_t fd[2] = {-1};
		   pipe(fd);
		   /*redirect stdout to fd[1], so that output behind will be redirect to 
		   	*kernel ioqueue*/
		   fd_redirect(1, fd[1]);
		   /*2. the first command*/
		   ...
		   fd_redirect(0, fd[0]);
		   /*3. command in between's input and output will all be redirected to ioqueue*/
   ...
   panic("my_shell: should not be there\n");
}
